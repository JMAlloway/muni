...
AGENCY_NAME = "City of Grove City"
LIST_URL = "https://www.grovecityohio.gov/Bids.aspx"
...
def _parse_date_mmddyyyy(text_in: str) -> Optional[datetime]:
    ...
    If we see 'Open Until Contracted' or similar, return None.
    """
    if not text_in:
        return None
...
async def _fetch_detail_description_and_dates_and_attachments(
    ...
    try:
        ...
        logger.warning(f"Grove City detail fetch failed {url}: {e}")
    ...
    # Try finding a main description block
    ...
    # Collect attachments (PDF, DOC, XLS, ZIP, etc.)
    attachment_urls: List[str] = []
    for a in soup.find_all("a", href=True):
        ...
        if any(
            ...
            if href.startswith("http://") or href.startswith("https://"):
                ...
                attachment_urls.append("https://www.grovecityohio.gov" + href)
            else:
                attachment_urls.append("https://www.grovecityohio.gov/" + href.lstrip("/"))
...
def _normalize_detail_url(href: str) -> str:
    """
    Turn relative CivicPlus hrefs into absolute URLs.
    Handles:
      "/Bids.aspx?BidID=23"
      "Bids.aspx?BidID=23"
      "https://www.grovecityohio.gov/Bids.aspx?BidID=23"
    """
    if href.startswith("http://") or href.startswith("https://"):
        return href
    if href.startswith("/"):
        return "https://www.grovecityohio.gov" + href
    return "https://www.grovecityohio.gov/" + href.lstrip("/")
...
def _extract_rows_from_table(soup: BeautifulSoup) -> List[tuple]:
    ...
    Rules:
    - Only consider tables that include 'Bid Title' or 'Closing Date'
    - Only keep <tr> rows that:
        * have >=2 <td> cells
        * contain an <a> with href containing 'bidID=' (actual bid rows)
      This filters out:
        - breadcrumbs ("Home")
        - subscribe / signup links
...
def _extract_rows_from_cards(soup: BeautifulSoup) -> List[tuple]:
    """
    CivicPlus sometimes renders bids as cards:
        <div class="listItemsRow bid">
            <div class="bidTitle">
                <span><a href="bids.aspx?bidID=23">Real Title ...</a></span><br>
                ...
                [<a href="bids.aspx?bidID=23">Read on</a>]
            </div>
            <div class="bidStatus"> ... </div>
        </div>

    Same issue as Gahanna: two links with bidID=...
    We only want the FIRST one in that card.
    """
    rows: List[tuple] = []
...
async def _scrape_listing_page() -> List[RawOpportunity]:
    """
    Scrape Grove City bid listing page and return RawOpportunity objects.
    """
    ...
    # Try table layout first, then fall back to card layout.
    rows = _extract_rows_from_table(soup)
    if not rows:
        rows = _extract_rows_from_cards(soup)

    if not rows:
        # CivicPlus often shows a "no open bid postings" message when empty
        if "no open bid postings" in page_text or "no open bids" in page_text:
            logger.info("Grove City: no open bids at this time.")
            return []
        else:
            logger.warning("Grove City: page parsed but no recognizable bid rows found.")
            return []

    # Dedupe by URL (in case same bid appears in both table and card layouts)
    seen = set()
    ...
    async with aiohttp.ClientSession() as session:
        for (title, detail_url, row_text) in rows:
            # Try guessing due date from the listing row (e.g. "Closing Date: 10/13/2025 3:00 PM")
            ...
            best_due = due_dt_detail or due_dt_row  # may be None
    ...
    logger.info(f"Grove City: scraped {len(out)} bid(s).")
    return out
...
async def fetch() -> List[RawOpportunity]:
    """
    Compatibility for runner.py (runner calls city_grove_city.fetch()).
    """
