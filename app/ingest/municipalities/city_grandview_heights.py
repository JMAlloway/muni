...
AGENCY_NAME = "City of Grandview Heights"
LIST_URL = "https://www.grandviewheights.gov/Bids.aspx"
BASE_URL = "https://www.grandviewheights.gov"
...
async def _fetch_detail_description_and_dates_and_attachments(
    ...
    try:
        ...
        logger.warning(f"Grandview Heights detail fetch failed {url}: {e}")
...
async def _scrape_listing_page() -> List[RawOpportunity]:
    ...
    if not rows:
        if "no open bid postings" in page_text or "no open bids" in page_text:
            logger.info("Grandview Heights: no open bids at this time.")
            return []
        else:
            logger.warning("Grandview Heights: page parsed but no recognizable bid rows found.")
            return []
    ...
    logger.info(f"Grandview Heights: scraped {len(out)} bid(s).")
    return out
