...
AGENCY_NAME = "City of Whitehall"
LIST_URL = "https://www.whitehall-oh.us/Bids.aspx"
BASE_URL = "https://www.whitehall-oh.us"
...
async def _fetch_detail_description_and_dates_and_attachments(
    ...
    try:
        ...
        logger.warning(f"Whitehall detail fetch failed {url}: {e}")
...
async def _scrape_listing_page() -> List[RawOpportunity]:
    ...
    if not rows:
        if "no open bid postings" in page_text or "no open bids" in page_text:
            logger.info("Whitehall: no open bids at this time.")
            return []
        else:
            logger.warning("Whitehall: page parsed but no recognizable bid rows found.")
            return []
    ...
    logger.info(f"Whitehall: scraped {len(out)} bid(s).")
    return out
